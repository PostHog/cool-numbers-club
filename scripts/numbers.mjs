 ...
 *
 * Most entries are generated by rule so the club keeps growing on its own as the
 * repo burns through PR numbers -- pass a higher `max` and new achievements
 * appear. Hand-written entries in MEMES/MATH cover the numbers
 * that are only cool because humans decided they were.
 */

/**
 * The club stops at the configured ceiling -- a million by default. Past that
 * the joke has run its course, and a round power of ten is a fine place for the
 * odometer to retire. Whatever the ceiling is, that number becomes the one and
 * only Singularity.
 */
export const CEILING = config.ceiling
